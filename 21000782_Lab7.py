...
# name and ID 
text = "Long Adarisa Binti Bahari 21000782"
st.markdown(f"""
    <div style="border: 2px solid #FFB6C1; padding: 10px; border-radius: 5px; background-color: #f9f9f9; text-align: center;">
        <strong>{text}</strong>
    </div>
    """, unsafe_allow_html=True)
st.title("Lab 7: Interactive Map Visualization")
...
if map_type == "Map Cosmetics":
    st.header("Map Cosmetics")
    fig, ax = plt.subplots(figsize=(10, 6))
    gdf.plot(ax=ax, color='skyblue', edgecolor='black', linewidth=0.5)
    ax.set_title("Styled Map", fontsize=15, fontweight="bold")
...
# 2. Interactive Map Visual Hierarchy Example
    ...
    if country_col:
        # 2. Interactive Map Visual Hierarchy Example
        st.header("Interactive Map Visual Hierarchy")
...
# 4. Interactive Geo Projection Example
    ...
    st.header("Interactive Geo Projection")
